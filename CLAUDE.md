...
- **Env vars**: `DATABASE_URL` (pooled, used at runtime), `DATABASE_URL_UNPOOLED` (direct, used for migrations), `SESSION_SECRET` (at least 32 characters), optional `NEXT_PUBLIC_API_BASE_URL`, and `ADMIN_INVITE_CODES` (a comma-separated server-side allowlist for admin sign-up) go in a local `.env` or `.env.local` (gitignored, never commit real credentials) — see `.env.example` for the expected format.
