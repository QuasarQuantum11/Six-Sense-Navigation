...
- **Current schema** (expect this to grow as features land): `students`, `admins`, `feedback`, `buildings`, `timetables`, `timetable_buildings` — a junction table for the many-to-many relationship between timetables and buildings, ordered by `position`. See `lib/*/schema.ts` for the source of truth.
- **Auth**: students can sign up and students/admins can log in using scrypt-hashed passwords. A signed, HttpOnly session cookie carries the minimum user ID/username/role payload; server-side DAL checks protect student and admin data. Email-verification fields exist, but email delivery/verification is not implemented yet.
- **Env vars**: `DATABASE_URL` (pooled, used at runtime), `DATABASE_URL_UNPOOLED` (direct, used for migrations), `SESSION_SECRET` (at least 32 characters), and optional `NEXT_PUBLIC_API_BASE_URL` go in a local `.env` (gitignored, never commit real credentials) — see `.env.example` for the expected format.
